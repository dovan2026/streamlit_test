...
# 방법 1: columns.difference 사용 -> 지정한 칼럼을 제외하고 가져오기(단, 컬럼 순서가 바뀔 수 있음)
df_line = df_off[df_off.columns.difference(['날짜', '연번', '역번호', '역명', '구분', '합계'])]
st.dataframe(df_line)

# 방법 2: drop으로 지정한 컬럼 제거 (순서 유지 됨)
# df_line = df_off.drop(columns=['날짜', '연번', '역번호', '역명', '구분', '합계'], axis=1)

st.dataframe(df_line)

# 4. 데이터 구조 변환(melt)
st.text("4. 아래 방법으로 데이터프레임 변환하여 저장 & 확인")
st.caption('melt 함수 사용 unpivot: identifier='호선', var_name='시간',
           value_name='인원수')
